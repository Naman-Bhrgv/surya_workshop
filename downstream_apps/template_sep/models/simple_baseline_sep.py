...
class LogRegressionFlareModel(nn.Module):
    ...
    def forward(self, x: dict) -> torch.Tensor:
        ...
        ts = ts.abs().mean(dim=[3, 4])
    
        # Rearrange in preparation for linear layer
        ts = rearrange(ts, "b c t -> b (c t)")
    
        # Read scalar from batch dict instead of separate argument
        extra = x["flare_strength"].to(ts.device).float().unsqueeze(1)  # (B,) -> (B, 1)
        ts = torch.cat([ts, extra], dim=1)  # (B, C*T + 1)
    
        return torch.sigmoid(self.linear(ts))