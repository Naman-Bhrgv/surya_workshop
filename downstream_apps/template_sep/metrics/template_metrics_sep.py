...
class SEPMetrics:
    ...
    def train_metrics(
        ...
        pred_labels = (preds.reshape(-1) >= 0.5).float()
        
        output_metrics["accuracy"] = (
            pred_labels == target.reshape(-1)
        ).float().mean()
    ...
    self, preds: torch.Tensor, target: torch.Tensor
        ...
        """
        Calculate accuracy for binary classification validation.
    
        Args:
            preds (torch.Tensor): Model predicted probabilities.
            target (torch.Tensor): Ground truth labels.
    
        Returns:
            tuple[dict[str, torch.Tensor], list[float]]:
                - Dictionary containing validation accuracy.
                - List containing the metric weight.
        """
        ...
        output_metrics["accuracy"] = (pred_labels == target).float().mean()
        output_weights.append(1)
